...
unseenMailList:function(id){
	...
	detailMailParallelUrl: function(args) {
		...
		if (fid && uid) {
			req = {
				url: DA.vars.spCgiRdir + "/sp_ajx_api.cgi?proc=contents_json&func=mail&sub=detail&fid=" + fid + "&uid=" + uid + "&srid=" + srid + "&keep=1",
	...
	switchOrgMailParallelUrl: function(org_mail_gid) {
		var org_mail_gid = org_mail_gid;
		var org_changed = "true";
		var req;

		if (fid && uid) {
			req = {
				url: DA.vars.spCgiRdir + "/sp_ajx_api.cgi?proc=contents_json&func=mail&sub=folder&mail_func=list&org_changed=" + org_changed + "&org_mail_gid=" + org_mail_gid,
				key: null
			};
		}
	...
	seenMailParallelUrl: function(args) {
		...
		if (fid && uid) {
			req = {
				url: DA.vars.spCgiRdir + "/sp_ajx_api.cgi?proc=contents_json&func=mail&sub=detail&fid=" + fid + "&uid=" + uid + "&srid=" + srid + "&mail_func=seen",
	...
	updateMailParallelUrl: function() {
		...
		var req = {
			url: DA.vars.spCgiRdir + "/sp_ajx_api.cgi?proc=contents_json&func=mail&sub=folder&mail_func=update",
			key: key,
			expire: 30,
	...
	switchFolders: function(type, slide, org_mail_gid) {
		var me = this;
		var args = {
			fid: 0,
			mail_func: type
		...
		var onExecute = function() {
			if (org_mail_gid) {
				readCon.execute([me.switchOrgMailParallelUrl(org_mail_gid)]);
			} else {
